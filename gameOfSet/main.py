...
class Card:
    ...
    def __init__(self, n: int, s: int, c: int, f: int) -> None:
        """Initiate."""
        self.number = n
        self.shape = s
        self.color = c
    ...
    def find_card_pic_file(self) -> str:
...
def set_no_set(clicks: list) -> None:
...
def check_set(clicks: list) -> bool:
...
def collisions(event: pygame.event) -> None:
...
def draw_texts() -> None:
    ...
    # Text for package and number of cards in it.
    ...
    # Text for sets_found and number of sets in it.
...
def manage_messages(message_key) -> None:
...
def main() -> None:
    ...
    pygame.init()  # Please ignore "Module 'pygame' has no 'init'...".
    pygame.font.init()
